...
ot_settings = {
    "server": "http://server.akagiot.org/",
    "online": False,
...
def online_settings_init():
    ...
    1. ``AKAGI_BOT_CONFIG`` env var — pointer to a JSON file written by the
       Akagi v3 host. Recognised keys (``server``, ``online``, ``api_key``)
       override the in-module defaults; everything else is ignored.
    2. ``ot_settings.json`` next to ``model.py`` — pre-v3 legacy fallback.
    3. The hard-coded defaults declared above (offline, dummy URL/key).

    Source 1 wins when set so the frontend's "Bot settings" form is the
    single source of truth post-v3.
    """
    global ot_settings
    ...
    if cfg_path and os.path.isfile(cfg_path):
        try:
            ...
            for key in ('server', 'online', 'api_key'):
                if key in resolved:
                    ot_settings[key] = resolved[key]
            return
    ...
    legacy = pathlib.Path(__file__).parent / 'ot_settings.json'
    if legacy.exists():
        try:
...
def _build_engine() -> MortalEngine:
    ...
    # 优先加载完整版权重（mortal_full.pth，如 Mortal-S42 192ch×40blocks），
    # 加载失败自动回退捆绑的 lite 版（mortal.pth），保证教练永不因模型问题停摆
    here = pathlib.Path(__file__).parent
    candidates = [p for p in (here / "mortal_full.pth", here / "mortal.pth") if p.exists()]

    last_err: Exception | None = None
    for control_state_file in candidates:
        try:
            state = torch.load(control_state_file, map_location=device, weights_only=False)
            config = state.get('config')
            if config is None:
                # 外部权重（如 Mortal-S42）不内嵌 config，按 model-identity.json 已验证的规格指定
                config = {
                    'control': {'version': 4},
                    'resnet': {'conv_channels': 192, 'num_blocks': 40},
                }
            mortal = Brain(version=config['control']['version'], conv_channels=config['resnet']['conv_channels'], num_blocks=config['resnet']['num_blocks']).eval()
            dqn = DQN(version=config['control']['version']).eval()
            mortal.load_state_dict(state['mortal'])
            dqn.load_state_dict(state['current_dqn'])

            sys.stderr.write(
                f"[coach] model loaded: {control_state_file.name} "
                f"v{config['control']['version']} "
                f"{config['resnet']['conv_channels']}ch x {config['resnet']['num_blocks']}blocks\n"
            )
            sys.stderr.flush()
            ...
            last_err = exc
            sys.stderr.write(f"[coach] model load failed for {control_state_file.name}: {exc}\n")
            sys.stderr.flush()
    raise last_err
