...
| 面板跑出屏幕 | 点左下角 `教练 N帧` 角标：面板一键归位；改窗口大小/换分辨率时位置也会自动夹回可视区 |
...
### v1.24.0（油猴脚本 @version 1.24.0，累计 v1.20~v1.24 改动）
- **鸣牌决策反馈**：吃/碰/明杠/荣和/跳过之后，账本里补一行「你选择了吃（Mortal 第 1 名 84.99%）」，右侧表情按选项数映射档位（2 选 = 最好/最差，3 选 = 最好/中/最差）
- **候选项卡片化**：每个候选独立成卡片（首选金色描边），牌面在上、期望值在下，横排对齐更整齐
- **反馈表情放大**：36px → 54px，切牌/鸣牌正确度一眼可见
- **面板位置自愈**：窗口缩放、分辨率变化时自动把面板夹回可视区域；点左下角 `教练 N帧` 角标一键归位
- **三栏布局微调**：左预留位收窄（36 → 12px）、右表情栏加宽（44 → 62px），反馈行不再显示多余分隔符
